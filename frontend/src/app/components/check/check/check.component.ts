...
import {
  CheckedOccurrence,
  CheckedOccurrenceLabel,
  Occurrence,
} from "./checked-occurrence";
...
export class CheckComponent implements OnInit {
  ...
  ngOnInit() {
    this.authService
      .getUser()
      .subscribe((user) => (this.userId = user.principal.user.id));
  ...
  setCheckedType(checkType: CheckedOccurrenceLabel): void {
    ...
    if (this.isReady) {
      ...
      if (
        checkType === CheckedOccurrenceLabel.SENTENCE_ERROR ||
        checkType === CheckedOccurrenceLabel.PRIVATE
      ) {
        this.httpClient
          .post(
            `${environment.url}user_group/${this.groupId}/element/${occurrence.dataElementId_1}/checked?type=${checkType}`,
            {}
          )
          .subscribe((value) =>
            this.snackBarService.openMessage("successfully updated element.")
          );
      } else {
        const cta = new CheckedOccurrence(occurrence.id, checkType);
        this.httpClient
          .post(
            `${environment.url}user_group/${this.groupId}/occurrence/check`,
            cta
          )
          .subscribe();
        ...
        if (
          this.carousel.carousel.activeIndex ===
          ...
          this.isReady = false;
          this.loadAudioBlob(
            this.occurrences[this.carousel.carousel.activeIndex + 1]
          );
  ...
  private getTenNonLabeledTextAudios() {
    this.httpClient
      .get<Array<Occurrence>>(
        `${environment.url}user_group/${this.groupId}/occurrence/next`
      )
  ...
  private getNonLabeledTuple() {
    this.tuple = new TupleDto(1, 2, TupleType.TEXT_TEXT, 0, 0, 0, 0);
  }

  private loadAudioBlob(occurrence: Occurrence): void {
    this.httpClient
      .get(
        `${environment.url}user_group/${this.groupId}/occurrence/audio/${occurrence.dataElementId_2}`,
        { responseType: "blob" }
      )
      .subscribe((resp) => {
