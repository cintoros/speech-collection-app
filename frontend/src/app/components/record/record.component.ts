...
import {ExcerptComponent} from 'src/app/components/record/excerpt/excerpt.component'
    ...
    @Component({selector: 'app-record', templateUrl: './record.component.html', styleUrls: ['./record.component.scss']}) export class RecordComponent implements OnInit {textDto: TextDto = null; isRecording = false; blobUrl: SafeUrl; recordingQuality = RecordingQuality; recordingNoiseLevel = RecordingNoiseLevel; selectedQuality = RecordingQuality.INTEGRATED; selectedNoiseLevel = RecordingNoiseLevel.MODERATE_NOISE;
                                                                                                                                                                         // @ts-ignore
                                                                                                                                                                         private mediaRecorder: MediaRecorder; private audioChunks = []; private groupId = 1; private elapsedTime = 0; private numberObservable: Observable<number>; private subscription: Subscription;

                                                                                                                                                                         constructor(private snackBarService: SnackBarService, private detector: ChangeDetectorRef, private httpClient: HttpClient, private domSanitizer: DomSanitizer, private userGroupService: UserGroupService) {
    ...
    this.check(CheckedDataElementType.SENTENCE_ERROR).subscribe(() => {
      ...
      this.getNext();
                                                                                                                                                                         ...
                                                                                                                                                                         isReady = () => this.audioChunks.length > 0; private check = (type: CheckedDataElementType) => this.httpClient.post<void>(`${environment.url}user_group/${this.groupId}/element/${this.textDto.id}/checked?type=${type}`, {});
